#!/usr/bin/env python
"""
plot_profiles_pcrb.py

Reads profiling data from "profile_log_pcrb.csv", replaces extremely high values in each measurement column
...
# Read the CSV file.
df = pd.read_csv("profile_log_pcrb.csv")
print(len(df))
...
plt.suptitle("PCRB Profiling Summary", fontsize=16)
plt.savefig("profiling_summary_pcrb.png")
plt.show()
